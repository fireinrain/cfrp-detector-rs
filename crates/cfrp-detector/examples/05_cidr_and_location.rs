...
use cfrp_detector::{CidrSource, CloudflareRanges, LocationStore, LocationSource};
...
fn main() -> anyhow::Result<()> {
    ...
    let ip_cases: Vec<(IpAddr, bool, &str)> = vec![
        // (IP, 是否属于 CF, 说明)
        (IpAddr::V4(Ipv4Addr::new(1, 1, 1, 1)),       true,  "Cloudflare 公共 DNS"),
        (IpAddr::V4(Ipv4Addr::new(104, 16, 0, 1)),    true,  "典型 CF 104.16/12 段"),
        (IpAddr::V4(Ipv4Addr::new(172, 67, 0, 1)),    true,  "典型 CF 172.64/13 段"),
        (IpAddr::V4(Ipv4Addr::new(8, 8, 8, 8)),       false, "Google DNS (非 CF)"),
        (IpAddr::V4(Ipv4Addr::new(203, 0, 113, 1)),   false, "APNIC 实验段"),
        (IpAddr::V6(Ipv6Addr::new(0x2606, 0x4700, 0, 0, 0, 0, 0x1111, 1)), true, "CF DNS IPv6 2606:4700::/32"),
        (IpAddr::V6(Ipv6Addr::new(0x2001, 0x4860, 0, 0, 0, 0, 0x8888, 0x8888)), false, "Google DNS IPv6"),
    ];
    ...
    for (ip, expected, note) in &ip_cases {
        ...
        let ok = if actual == *expected { "✅" } else { "❌ MISMATCH" };
        println!("{:<50} {:<8} [{}] {}",
            ip.to_string(),
            if actual { "YES" } else { "no" },
            ok, note);
        if actual == *expected { hit += 1; }
    }
    println!("\nCIDR 归属测试: {}/{} 通过\n", hit, ip_cases.len());
    ...
    let colos = ["LAX", "SFO", "DFW", "IAD", "JFK", "YYZ",
                 "LHR", "CDG", "FRA", "AMS", "MAD", "WAW", "IST", "DXB",
                 "NRT", "HND", "KIX", "ICN", "SIN", "HKG", "BKK", "SYD",
                 "GRU", "JNB", "SJC", "PEK", "SHA", "CAN"];

    println!("{:<8} {:<20} {:<16} {:<18} {}", "CODE", "CITY", "REGION", "COUNTRY", "NOTES");
    println!("{}", "-".repeat(100));
    ...
    for code in colos {
        match loc.lookup(code) {
            ...
            None => println!("{:<8} {:<20} {:<16} {:<18} (离线空表 → 未命中)",
                code, "-", "-", "-"),
        }
    }
    println!("\n地理位置查询: {}/{} 命中 (预期 0, 离线示例)", found, colos.len());
    ...
    println!("目标 IP: {}", ip);
    println!("  CIDR 命中? → {}", if ranges.contains(ip) { "✅ 属于 Cloudflare" } else { "❌ 非 Cloudflare" });
