...
async fn main() -> anyhow::Result<()> {
    ...
    println!("🧪 默认 DetectorConfig: 并发={}, 超时={:?}, governor={}",
        cfg.max_concurrency,
    ...
    ].into_iter().enumerate().map(|(i, t)| BatchTarget { target: t, id: i }).collect();
    ...
    for r in &results {
        ...
        match (&r.result, &r.error) {
            (Some(res), _) => {
                let colo = res.edge_info.as_ref()
                    .and_then(|e| e.colo_code.clone()).unwrap_or("-".into());
                let lat = res.edge_info.as_ref().and_then(|e| e.latency)
                    .map(|d| format!("{}ms", d.as_millis())).unwrap_or("-".into());
                let conf = format!("{:?}", res.confidence).to_uppercase();
                println!("{:<24} {:<8} {:<6} {:<6} {:<10} {} / {}",
                    key,
                    yn(res.is_cloudflare_edge), yn(res.is_tls), yn(res.is_usable),
                    conf, colo, lat);
    ...
    let n_edge = results.iter().filter(|r|
        r.result.as_ref().map(|x|x.is_cloudflare_edge).unwrap_or(false)).count();
    println!("\n📊 {}/{} 目标判定为 Cloudflare 边缘", n_edge, results.len());
    Ok(())
}
fn yn(v: bool) -> &'static str { if v { "✅" } else { "·" } }