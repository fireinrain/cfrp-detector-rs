...
use cfrp_detector::{
    AdaptiveConfig, BatchResult, BatchTarget, ConnectorConfig, Detector, DetectorConfig, ProbeConfig,
...
fn build_cfg() -> DetectorConfig {
    ...
    let _demo = AdaptiveConfig {
        enabled: true,
        initial: 4, min: 1, max: 32, window: 8,
    };
...
async fn async_run() -> anyhow::Result<()> {
    ...
    println!("   TLS Session     = {} (size={})", cfg.probe.tls_session_cache, cfg.probe.tls_session_cache_size);
    ...
    let targets: Vec<BatchTarget> = raw_targets.clone().into_iter().enumerate().map(|(i, t)| BatchTarget { target: t, id: i }).collect();
    ...
    let candidates: Vec<Target> = results.iter()
        .filter(|r| r.result.as_ref().map(|x|x.is_usable).unwrap_or(false))
        .map(|r| r.target.clone())
        .collect();
    println!("   → {}/{} 候选节点进入测速\n", candidates.len(), raw_targets.len());
    ...
    println!("   配置: timeout={:?}, threads_per_target={}, concurrency={}\n",
    ...
    let tester = SpeedTester::new(connector_cfg, true, cfg.probe.default_sni.clone(), cfg.probe.default_sni.clone())?;
    let speed_results = tester.test_batch(&candidates, "/cdn-cgi/trace", &st_cfg).await;
