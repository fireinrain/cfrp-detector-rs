...
async fn main() -> anyhow::Result<()> {
    ...
    let cb = move |p: BatchProgress| {
        let total = p.total;
        let pct = if total > 0 {
            (p.completed as f64 / total as f64) * 100.0
        } else {
            0.0
        };
        // 每完成 5 个 或 全部完成 时刷新
    ...
    // 置信度分布
    use cfrp_detector::Confidence::{High, Low, Medium};
